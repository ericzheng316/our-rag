#!/usr/bin/env bash
# ACEC v7 G5: 1 episode, 16 questions x 8 samples, frozen learned selector.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$PERSIST_ROOT/logs/acec_v7_validation_$(date -u +%Y%m%dT%H%M%SZ)}"
L="${L:-20}"
K="${K:-5}"
...
PYTHONPATH="$REPO_ROOT/rag/src:$REPO_ROOT/rag/train" \
    "$PYTHON" "$REPO_ROOT/rag/train/tests/test_learnable_set_selector_v7.py"
...
"$PYTHON" "$REPO_ROOT/rag/train/validate_smoke_v7.py" \
    --input "$OUTPUT_ROOT/online_metrics_v7.jsonl" \
    --output "$OUTPUT_ROOT/v7_smoke_gate.json"
