#!/usr/bin/env bash
# Isolated ACEC-v5 16q x 8-sample smoke run.  Start the persistent retriever
# on RETRIEVER_GPU before invoking this script; v1-v4 entry points are untouched.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$PERSIST_ROOT/logs/acec_v5_validation_$(date +%Y%m%d_%H%M%S)}"
...
echo "[preflight] artifact=$ACEC_ARTIFACT_V5"
...
echo "ACEC-v5 validation log: $OUTPUT_ROOT/train.log"
