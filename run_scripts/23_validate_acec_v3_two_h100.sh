#!/usr/bin/env bash
# Isolated ACEC-v3 validation run.  The persistent retriever must already be
# serving on RETRIEVER_GPU; this script only uses TRAIN_GPU for vLLM/training.
...
ACEC_ARTIFACT_V3="${ACEC_ARTIFACT_V3:-}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$HOME/logs/acec_v3_validation_$(date +%Y%m%d_%H%M%S)}"
...
if [[ -z "$ACEC_ARTIFACT_V3" || ! -f "$ACEC_ARTIFACT_V3" ]]; then
    echo "Set ACEC_ARTIFACT_V3 to a calibration-v3 artifact" >&2
    exit 2
fi
...
if [[ "$RUN_UNIT_TESTS" == "1" ]]; then
    PYTHONPATH="$REPO_ROOT/rag/src" \
        "$PYTHON" "$REPO_ROOT/rag/src/belief/acec/tests/test_calibration_v3.py"
...
echo "ACEC-v3 validation log: $OUTPUT_ROOT/train.log"
