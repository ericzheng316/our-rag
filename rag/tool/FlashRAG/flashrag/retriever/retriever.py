...
#     elif self.backend == "bm25s":
#         query_tokens = self.tokenizer.tokenize(query, return_as='tuple', update_vocab=False)
#         raw_results, scores = self.searcher.retrieve(query_tokens, k=num)
        
#         # 将结果转换为标准格式，类似于DenseRetriever的处理方式
#         results = []
#         for batch_idx, batch_raw_results in enumerate(raw_results):
#             batch_results = []
#             for doc_idx in batch_raw_results:
#                 # 假设doc_idx是文档索引，获取实际文档内容
#                 if isinstance(doc_idx, (int, np.integer)):
#                     # 如果是索引，加载文档
#                     doc = self.corpus[doc_idx]
#                     batch_results.append(doc)
#                 else:
#                     # 如果已经是文档对象，确保是标准dict
#                     if isinstance(doc_idx, dict):
#                         batch_results.append(dict(doc_idx))
#                     else:
#                         # 如果是其他类型，尝试转换为字典
#                         batch_results.append({"content": str(doc_idx)})
