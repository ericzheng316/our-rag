...
def get_args():
    ...
    parser.add_argument('--retrieve_url', type=str, default=None, help="Retrieve url (not needed in distractor mode).")
    parser.add_argument('--split_url', type=str, default=None, help="Split url (not needed in distractor mode).")
    ...
    parser.add_argument('--dev_file', type=str, default='dev.jsonl',
                        help="Dev filename under each dataset dir (e.g. dev_mini.jsonl).")
...
def load_distractor_ctx(distractor_file):
    """Load {question: [para_str, ...]} from a JSONL with distractor_paras field."""
    ctx = {}
    with open(distractor_file, encoding='utf-8') as f:
        for line in f:
            d = json.loads(line)
            ctx[d['question']] = d['distractor_paras']
    print(f"[distractor] loaded {len(ctx)} questions from {distractor_file}")
    return ctx
...
def solve_main(args, ckpt, records, temperature=0, distractor_ctx=None, embedder=None):
    ...
    for turn in range(args.num_search_one_attempt):
        ...
        if distractor_ctx is not None:
            ...
            unique_queries = list(dict.fromkeys(query for _, query in retrive_list))
            if embedder is not None and unique_queries:
                query_embs = embedder.encode(unique_queries)          # (q, dim), already L2-normed
                query_emb_map = {q: query_embs[k] for k, q in enumerate(unique_queries)}
            else:
                query_emb_map = {}
