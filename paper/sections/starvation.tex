...
the training temperature from the SFT initialization---the policy that
training actually starts from---on a stratified question set (protocol
in \S\ref{sec:mechanism}). Table~\ref{tab:tie-ledger} shows the result:
ties are not an edge case but the dominant regime, 56--71\% of groups at
every depth. Depth changes the ties' \emph{character} more than their
...
(Fig.~\ref{fig:mechanism}A).\footnote{Our estimator is
...
non-zero exactly when coverage varies inside the group---and that
variation is concentrated where it is needed. At the SFT initialization,
