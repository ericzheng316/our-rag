...
| §2 Related Work | 三小节:检索RL一系 / 过程奖励一系 / 打平组缓解四派(丢弃·合成·改结构·我们=给内容) | related_notes.md 的划界表 | **已写 v1**(2026-08-31);冻稿前精读 Search-P1/AVSPO + 9 月补扫后复核 |
...
| §6 主结果 | Table 1 配对多种子(4hop 主端点)、Table 2 同底座阶梯、深度效应图、检索次数不升 | 旧配对 +7.2 先押上(用户决议 08-31);nsd2/3 到账后补 per-seed 配对表 | **已写 v2**(2026-09-03:四对配对表 Table 1 + 合并行;阶梯 RL 两行改显式种子均值;Fig 2 占位;v1 单对原句留注释) |
...
| §9 信号安置 | 训练奖励✓ / 离线诊断✓(选择性预测 5%→79%)/ 运行期控制✗ / 注入观测✗;措辞纪律:z=−1.80 写 "no reliable improvement" | J7/J8 材料全齐 | **已写 v1**(2026-08-31,0.75 页) |
...
|---|---|---|---|
| [+2.7, +7.2] 4hop 区间、"outcome-only counterpart" 措辞 | 配对 noshape 种子 1/2/3(同 seed、β=0) | **全部到账(nsd3 09-03)**;四对 4hop +3.6 z=3.62(三显式 +2.4 z=2.04) | **已回填**:§6 Table 1、intro 发现一与贡献三(09-03) |
